...
testing_dataset = shuffle(dataset, random_state=20)[:5000]
testing_dataset[:, :9] = zscore(testing_dataset[:, :9])
testing_labels = shuffle(labels, random_state=20)[:5000]
# count0 = 0
# for label in labels:
#     if label == 0:
#         count0 = count0 + 1
...
#start the program
print("********the problem start running**********\n")
...
if RBF_acc > KNN_acc:
    kernel = 'rbf'
    param = RBF_gamma
    print("we choose RBF kernel with gamma = " + str(param))
else:
    kernel = 'knn'
    param = KNN_numNei
    print("we choose KNN kernel with number of neighbor = " + str(param))
print("***************************************\n")
# N=1
# best_alpha = 0.8
# kernel = 'knn'
# param = 20
print("*********10-Fold training**********\n")
training_acc = 0
testing_acc = 0
pointer = 0
while pointer < 5000:
    ...
    for h in range(1, 5000):
        if h <= pointer or h >= (pointer + (500 * N)):
            training_labels[h] = -1
    label_spread = label_propagation.LabelSpreading(kernel=kernel,n_neighbors=param, gamma=param, alpha=best_alpha)
    label_spread.fit(training_dataset, training_labels)
    y_training_predicted = label_spread.predict(X_labeled)
    y_testing_predicted = label_spread.predict(training_dataset)
    count = 0
...
print("The average training accuracy is " + str(training_acc) + ", and the average testing accuracy on unseen data is " + str(testing_acc))
