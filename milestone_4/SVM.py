...
for i, (train_index, test_index) in enumerate(Kfold.split(features, label)):
    ...
    svm_model = svm.SVC(C=1, gamma=0.1, probability=True)
    ...
    neg_lpd_t = -np.mean(np.log(svm_model.predict_proba(X_train)[np.arange(len(X_train)), y_train]))
    neg_lpd_v = -np.mean(np.log(svm_model.predict_proba(X_test)[np.arange(len(X_test)), y_test]))
    nlpd_t[i] = neg_lpd_t
    nlpd_v[i] = neg_lpd_v
