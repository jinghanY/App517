...
accuracy_training_dec = np.zeros(n_splits)
accuracy_testing_dec = np.zeros(n_splits)
...
for i, (train_index, test_index) in enumerate(Kfold.split(features, label)):
    ...
    accuracy_training_dec[i] = accuracy_score(y_train, clf_tree.predict(X_train))
    accuracy_testing_dec[i] = accuracy_score(y_test, clf_tree.predict(X_test))
    neg_lpd_t = -np.mean(np.log(clf_tree.predict_proba(X_train)[np.arange(len(X_train)), y_train]))
    neg_lpd_v = -np.mean(np.log(clf_tree.predict_proba(X_test)[np.arange(len(X_test)), y_test]))
    nlpd_t[i] = neg_lpd_t
    nlpd_v[i] = neg_lpd_v
    a = np.log(clf_tree.predict_proba(X_test)[:, 0])
...
print("The average training accuracy of decision tree is " + str(np.mean(accuracy_training_dec)) + " and the average testing accuracy is " + str(np.mean(accuracy_testing_dec)))
